...
test("migration upgrades v0.1 data to v0.2 without changing project content", async () => {
  ...
  assert.equal(after.schema_version, "0.2.0");
  ...
  assert.equal(
    JSON.parse(second.stdout).warnings.some((warning) =>
      /already uses schema 0\.2\.0/i.test(warning),
    ),
    true,
  );
});
